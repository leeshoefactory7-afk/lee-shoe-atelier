  ...
  return (
    <SiteLayout>
      <div className="container-lux py-10 md:py-16">
        <h1 className="font-serif text-3xl md:text-5xl">Shopping Cart</h1>
        {!items.length ? (
          <div className="mt-10 md:mt-16 text-center py-16 border border-dashed border-border">
          ...
          <div className="mt-8 md:mt-10 grid md:grid-cols-[1.6fr_1fr] gap-8 md:gap-14">
            <div className="divide-y divide-border">
              {items.map((it: any) => (
                <div
                  key={`${it.productId}-${it.size}-${it.color}`}
                  className="grid grid-cols-[80px_minmax(0,1fr)_auto] sm:grid-cols-[100px_minmax(0,1fr)_auto] gap-3 sm:gap-4 py-5 sm:py-6 items-start"
                >
                  <img src={it.image} alt={it.name} className="w-20 sm:w-24 h-24 sm:h-28 object-cover bg-muted" />
                  <div className="min-w-0">
                    <Link to="/products/$slug" params={{ slug: it.slug }} className="font-serif text-base sm:text-xl hover:text-accent line-clamp-2 block">
                      {it.name}
                    </Link>
                  ...
                  <div className="text-right font-medium text-sm sm:text-base whitespace-nowrap">{formatPrice(it.price * it.quantity)}</div>
                </div>
              ))}
            </div>
            <aside className="bg-muted/50 p-6 sm:p-8 h-fit">
